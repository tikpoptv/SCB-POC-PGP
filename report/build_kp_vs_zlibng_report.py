...
# pseudo-labels สำหรับหน้า "อัพเกรดไลบรารีช่วยกี่เท่า" (แกน y เป็นเท่า ไม่ใช่ ms)
base.LABEL_DISP.update({"go-gain": "Go: stdlib → klauspost", "java-gain": "Java: libz → zlib-ng"})
base.LABEL_COLOR.update({"go-gain": C_KP, "java-gain": C_NG})
...
# ── หน้า 4: อัพเกรดไลบรารีช่วยกี่เท่า (gain ในภาษาตัวเอง) ────────────────────
def page_gains(rows):
    """Go: stdlib → klauspost (คู่จากรอบ FULL เดิม รันพร้อมกัน)
    Java: libz ระบบ → zlib-ng (java จากรอบเดิม, java-zlibng จากรอบนี้ — cross-run ~1–2% noise)"""
    def go_gain(r):
        return (r["std"] / r["kp"]) if (r["std"] and r["kp"]) else None

    def jv_gain(r):
        return (r["jv"] / r["ng"]) if (r["jv"] and r["ng"]) else None

    # median ต่อสกุลไฟล์
    by_ft = {}
    for r in rows:
        by_ft.setdefault(sc_filetype(r["name"]), []).append(r)
    ft_order = ["txt", "csv", "pdf", "zip", "xlsx", "dat", "mixed"]
    ft_disp = {"mixed": "mixed (count/many/conc)"}
    ...
    for ft in ft_order:
        ...
        if not rs:
            continue
        groups.append((ft_disp.get(ft, ft), [
            ("go-gain", med([go_gain(r) for r in rs])),
            ("java-gain", med([jv_gain(r) for r in rs])),
        ]))
    chart_ft = svg_grouped(groups, "อัพเกรดไลบรารีแล้วเร็วขึ้นกี่เท่า — median ต่อสกุลไฟล์",
                           "แกน = เท่า (สูง = คุ้ม) · >1 = เร็วขึ้น · เทียบในภาษาตัวเอง ไม่ใช่ข้ามภาษา",
                           unit="×", width=860)

    # เส้น gain ตามขนาดไฟล์ (sizegrad)
    by_name = {r["name"]: r for r in rows}
    xs_labels = [(kb, lab) for lab, kb in SG_SIZES]
    charts_sg = []
    for ext in SG_EXTS:
        series = {"go-gain": [], "java-gain": []}
        for lab, kb in SG_SIZES:
            r = find_sg(by_name, ext, lab)
            series["go-gain"].append((kb, go_gain(r) if r else None))
            series["java-gain"].append((kb, jv_gain(r) if r else None))
        charts_sg.append(svg_lines(series, xs_labels,
                                   f"Gain ตามขนาดไฟล์ — {ext}",
                                   "เท่า (log) · เหนือเส้น 1 = อัพเกรดแล้วเร็วขึ้น · hover ดูค่า",
                                   unit="×", width=820))

    med_go_all = med([go_gain(r) for r in rows])
    med_jv_all = med([jv_gain(r) for r in rows])
    med_go_txt = med([go_gain(r) for r in by_ft.get("txt", [])])
    med_jv_csv = med([jv_gain(r) for r in by_ft.get("csv", [])])

    legend = (
        '<div class="legend">'
        f'<span><span class="k" style="background:{C_KP}"></span>Go: stdlib → klauspost</span>'
        f'<span><span class="k" style="background:{C_NG}"></span>Java: libz → zlib-ng</span>'
        '</div>'
    )
    return f"""
<h2>อัพเกรดไลบรารีบีบอัด แต่ละภาษาได้อะไรกลับมา</h2>
<p>หน้านี้เทียบ<b>ในภาษาตัวเอง</b>: Go เดิม (stdlib zlib) → Go+klauspost กี่เท่า และ
Java เดิม (libz ระบบ) → Java+zlib-ng กี่เท่า (ตัวเลข = p50 เดิม ÷ p50 ใหม่, &gt;1 = เร็วขึ้น)</p>
{legend}
<div class="stats">
  <div class="sbox go-b"><div class="val">{med_go_all:.1f}×</div><div class="lbl">Go เร็วขึ้น (median ทั้ง 74 สนาม)</div></div>
  <div class="sbox green-b"><div class="val">{med_jv_all:.2f}×</div><div class="lbl">Java เร็วขึ้น (median ทั้ง 74 สนาม)</div></div>
  <div class="sbox go-b"><div class="val">{med_go_txt:.1f}×</div><div class="lbl">Go จุดพีค: txt (สูงสุด 6.1×)</div></div>
  <div class="sbox green-b"><div class="val">{med_jv_csv:.1f}×</div><div class="lbl">Java จุดพีค: csv (สูงสุด 3.2×)</div></div>
</div>
{chart_ft}
<div class="info">💡 <b>ทำไม Go ได้เยอะกว่า:</b> stdlib zlib ของ Go ช้ากว่า libz ของ C มาก
(จุดที่ทำให้ Go แพ้ Java แต่แรก) → เปลี่ยนเป็น klauspost คือกระโดดจาก "ช้าสุดในกลุ่ม" ขึ้นมานำ
ส่วน libz ของ Java เป็น C ที่เร็วอยู่แล้ว → zlib-ng เพิ่ม SIMD ได้อีกชั้นเดียว
gain เลยเล็กกว่า ยกเว้น csv ที่ SIMD ออกฤทธิ์เต็ม (2.9×)</div>
{"".join(charts_sg)}
<div class="hi">⚠️ <b>ไฟล์จิ๋ว (~1KB) อัพเกรดแล้วช้าลงทั้งคู่:</b>
Go 0.70–0.78× (klauspost มี setup cost ต่อ stream สูงกว่า stdlib), Java 0.88–0.98× —
ที่ขนาดนี้เวลาบีบอัดจริงแทบเป็นศูนย์ เหลือแต่ overhead · จุดคุ้มทุนอยู่ราว ≥64KB ทั้งสองภาษา
(mixed/count/many ที่ corpus ไฟล์เล็กปนกันก็โดนถ่วงด้วยเหตุเดียวกัน — Go เหลือ 1.7×, Java 1.1×)</div>
<div class="card">
<h3>อ่านตัวเลขนี้ยังไง</h3>
<ul>
<li><b>Go gain มาจากรอบ FULL เดิม</b> (stdlib กับ klauspost รันในรอบเดียวกัน — ไม่มี cross-run noise)</li>
<li><b>Java gain เทียบข้ามรอบ</b> (java จากรอบเดิม, java-zlibng จากรอบนี้) — noise ~1–2%
    วัดจาก go-klauspost ที่รันทั้งสองรอบ</li>
<li>gain สูง ≠ ชนะอีกภาษา — Java csv gain 2.9× ทำให้แซง klauspost แค่ ~7%
    เพราะฐานเดิมตามหลังอยู่มาก · ดูผลแข่งข้ามภาษาที่แท็บ "สรุปผล"</li>
</ul>
</div>
"""


# ── หน้า 5: วิธีทดสอบ + correctness ──────────────────────────────────────────
...
def build(new, old, out_path):
    ...
    pages = [
        ...
        ("gains", "⚡ อัพเกรดไลบรารีช่วยกี่เท่า", page_gains(rows)),
