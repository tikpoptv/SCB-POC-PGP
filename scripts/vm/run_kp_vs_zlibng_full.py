#!/usr/bin/env python3
"""
run_kp_vs_zlibng_full.py — FULL head-to-head: go-klauspost vs java+zlib-ng

คู่ชิงจากสองรอบก่อน: Go ตัวโม (klauspost) vs Java ตัวโม (LD_PRELOAD zlib-ng)
matrix เต็มเท่า FULL run เดิมทุกมิติ (74 scenarios) — ผล java/go-stdlib เดิม
join จาก results_klauspost_ab.json ได้เพราะ scenario ชื่อเดียวกัน/corpus seed เดิม

copy จาก run_klauspost_ab.py (716ad86) — แก้เฉพาะ: คู่ runner, output path,
ตัด .prom export (กันทับ pgp_bench.prom ของ FULL run), เพิ่ม verify_preload

ต้องมีบน VM: go-runner-klauspost, java-runner jar, ~/zlib-ng/libz.so.1 (zlib-compat)
รัน:  FULL=1 ROUNDS=5 WARMUP=3 POC_CORPUS=$HOME/corpus-kp \
      ZLIBNG_LIB=$HOME/zlib-ng/libz.so.1 python3 scripts/vm/run_kp_vs_zlibng_full.py
ผลออกที่: report/results_kp_vs_zlibng_full.json  (+ ตารางสรุปบน stdout)
"""
...
OUT_JSON = REPO / "report/results_kp_vs_zlibng_full.json"
...
# zlib-ng shared lib (build ด้วย --zlib-compat) — LD_PRELOAD ผ่าน env prefix
ZLIBNG_LIB = os.getenv("ZLIBNG_LIB", str(pathlib.Path.home() / "zlib-ng/libz.so.1"))
...
def verify_preload():
    """LD_PRELOAD ล้มแบบเงียบ (แค่ warning แล้วรันต่อด้วย libz เดิม) → ตัวเลข
    จะออกมาเท่า java เดิมโดยไม่มี error — ต้องดักก่อนรันจริง (เหมือน run_java_zlibng_ab)"""
    lib = pathlib.Path(ZLIBNG_LIB)
    if not lib.exists():
        print(f"❌ ไม่พบ zlib-ng ที่ {lib} — build ก่อน หรือตั้ง env ZLIBNG_LIB"); sys.exit(1)
    r = subprocess.run(["env", f"LD_PRELOAD={lib}", "java", "-version"],
                       capture_output=True, text=True, timeout=60)
    err = r.stderr or ""
    if "cannot be preloaded" in err or "ERROR: ld.so" in err:
        print(f"❌ LD_PRELOAD ล้มเหลว:\n{err.strip()}"); sys.exit(1)
    s = subprocess.run(["strings", str(lib)], capture_output=True, text=True)
    if "zlib-ng" not in (s.stdout or ""):
        print(f"⚠ {lib} ไม่มี marker 'zlib-ng' ข้างใน — build ผิดไฟล์?"); sys.exit(1)
    print(f"🔗 preload OK: {lib}")

def main():
    if not GO_KP.exists():
        print(f"❌ ไม่พบ {GO_KP} — รัน build_klauspost_ab.sh ก่อน"); sys.exit(1)
    if not JAR.exists():
        print(f"❌ ไม่พบ {JAR} — head-to-head ขาด java ไม่ได้"); sys.exit(1)
    verify_preload()

    print("=" * 64)
    print("FULL head-to-head:  go-klauspost vs java+zlib-ng")
    ...
    labels = ["go-klauspost", "java-zlibng"]
    ...
    # ไม่ export .prom — กันทับ pgp_bench.prom ของ FULL run เดิมบน Grafana

    # ── ตารางสรุป (ms ต่ำสุดต่อ label + speedup) ──────────────────────────
    print("\n" + "=" * 72)
    print("สรุป (p50 ms, ใช้ variant เร็วสุดของแต่ละ label)")
    print("=" * 72)
    hdr = f"{'scenario':14s} | {'go-kp':>10s} | {'java-zlibng':>11s} | {'ng vs kp':>9s}"
    print(hdr); print("-" * len(hdr))
    for sc_name, sc in results["scenarios"].items():
        kp = scenario_best_ms(sc.get("go-klauspost", {}))
        ng = scenario_best_ms(sc.get("java-zlibng", {}))
        def f(x, w=10): return f"{x:{w}.3f}" if x is not None else f"{'-':>{w}s}"
        sp = f"{ng/kp:8.2f}x" if (ng and kp) else f"{'-':>9s}"
        print(f"{sc_name:14s} | {f(kp)} | {f(ng, 11)} | {sp:>9s}")

    print(f"\n✅ ผลบันทึกที่: {OUT_JSON}")
    print("   (ng vs kp = java-zlibng/go-klauspost ; >1 = klauspost เร็วกว่า, <1 = zlib-ng แซง)")
